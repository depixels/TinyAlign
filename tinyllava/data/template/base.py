...
class Template:
    ...
    def encode(self, messages, tokenizer, mode='train'):
        ...
        rag_token = '<rag>'
        prompt = prompt.replace('<image>', '<image>' + rag_token)
        # input_ids = self.tokenizer_image_token(prompt, tokenizer, return_tensors='pt')
        input_ids = self.tokenizer_image_and_rag_tokens(prompt, tokenizer, return_tensors='pt')
        if mode == 'train':
    ...
    def _make_masks(self, labels, tokenizer, sep, eos_token_length, rounds):
        ...
        for rou in rounds:
            ...
            if len(parts) != 2:
                break
            parts[0] += sep
            round_len = len(self.tokenizer_image_and_rag_tokens(rou, tokenizer)) + eos_token_length
            instruction_len = len(self.tokenizer_image_and_rag_tokens(parts[0], tokenizer)) - 1
    ...
    @classmethod
    def tokenizer_image_and_rag_tokens(cls, prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, rag_token_index=RAG_TOKEN_INDEX, return_tensors=None):
        def _insert_separator(X, sep):
            return [ele for sublist in zip(X, [sep]*len(X)) for ele in sublist][:-1]

        # 按照 <image> 和 <rag> 分割 prompt
        prompt_chunks = []
        for chunk in prompt.split('<image>'):
            sub_chunks = chunk.split('<rag>')
            for i, sub_chunk in enumerate(sub_chunks):
                prompt_chunks.append(tokenizer(sub_chunk).input_ids)
                if i < len(sub_chunks) - 1:  # 在每个子块之间插入 <rag>
                    prompt_chunks.append([rag_token_index])
            if '<image>' in prompt and chunk != prompt.split('<image>')[-1]:
                prompt_chunks.append([image_token_index])  # 在每个块之间插入 <image>
