...
def load_pretrained_model(model_name_or_path, load_type='hf', load_8bit=False, load_4bit=False, device_map="auto",
    ...
    # config = json.load(open(os.path.join(model_name_or_path, 'config.json'), 'r'))
    # model_config.load_from_config(config)
    ...
    connector2_ckp_path = os.path.join(model_name_or_path, 'connector2/pytorch_model.bin')
    connector2_ckp = load_base_ckp_for_lora(connector2_ckp_path)
    model.connector2.load_state_dict(connector2_ckp, strict=False)
