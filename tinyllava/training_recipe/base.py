...
class BaseTrainingRecipe:
    ...
    def __call__(self, model):
        ...
        model.config.tune_type_connector2 = self.training_arguments.tune_type_connector2
    ...
    def add_args(self, model_args):
        ...
        if self.training_arguments.pretrained_model_path is not None:
            if 'tinyllava' not in self.training_arguments.pretrained_model_path:
                ...
                model_args['connector2'].update(dict(pretrained_connector2_path=os.path.join(self.training_arguments.pretrained_model_path, 'connector2')))
            elif self.training_arguments.pretrained_connector2_path:
                model_args['connector2'].update(
                    dict(pretrained_connector2_path=self.training_arguments.pretrained_connector2_path)
                )
    ...
    def tune_type_setting(self, model):
        ...
        model = self._connector2_tune_type_setting(model)
        return model    
    ...
    def _connector2_tune_type_setting(self, model):
        tune_type = self.training_arguments.tune_type_connector2.lower()
        assert tune_type in ('frozen', 'full', 'lora', 'qlora'), f'tune_type {tune_type} not supported in this training recipe!'   
        if tune_type == 'full':
            for p in model.connector2.parameters():
                p.requires_grad = True
        elif tune_type == 'frozen':
            for p in model.connector2.parameters():
                p.requires_grad = False
    ...
    def save(self, model, trainer):
        ...
        # save connector2
        connector2_state_dict = get_state_maybe_zero_3(model.connector2.named_parameters(), [''], False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            connector2_output_dir = os.path.join(self.training_arguments.output_dir, 'connector2')
            os.makedirs(connector2_output_dir, exist_ok=True)
            connector2_output_path = os.path.join(self.training_arguments.output_dir, 'connector2/pytorch_model.bin')
            torch.save(connector2_state_dict, connector2_output_path)
